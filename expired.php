...
$expired_meds = mysqli_query($conn, "SELECT * FROM medicines WHERE expiry_date < CURDATE() ORDER BY expiry_date ASC");
?>
...
<?php if(mysqli_num_rows($expired_meds) > 0){ ?>
<table>
    <tr>
        <th>ID</th>
        <th>Name</th>
    ...
    <?php while($row = mysqli_fetch_assoc($expired_meds)){ ?>
    <tr>
...
<?php } else { ?>
<p style="text-align:center;">No expired medicines.</p>
<?php } ?>
